/*
 ...
 Description   : Assumes that node 0 is a GPU node
                 Runs GPU code on the workers too
 To build use  : make 
 ============================================================================
 */
#include <stdio.h>
...
const double modelParams[]={2.477876056907E-009, 19376843.5401888}; // p_i and e_i
const double l= 1.2441013041013E-010;  // speed for a single transfer
...
int main (int argc, char *argv[])
{
  ...
  // node 0 is reserved for the GPU
  if (rank == 0)
    {      
	 ...
	 e[idx] = temp[1];
      ...
      // process part0 of the input on the GPU     
      int thrPerBlock = atoi (argv[3]);
      rijndaelEncryptFE (rk, keybits, iobuf, iobuf, assignment[0], thrPerBlock);
    ...
    {      
      ...
      rijndaelEncryptFE (rk, keybits, iobuf, iobuf, jobSize, thrPerBlock);
      rijndaelShutdown();
