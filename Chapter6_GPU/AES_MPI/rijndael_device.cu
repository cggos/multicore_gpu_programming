...
__constant__ u32 d_rk[RKLENGTH (256)];  // allocate the maximum possible needed space for the expanded key


#define RESHUFFLE(i) (((i & 0xFF) << 24) | \
                      ((i & 0xFF00) << 8) | \
                      ((i & 0xFF0000) >> 8) | \
                      ((i & 0xFF000000) >> 24 ))
...
__global__ void rijndaelGPUEncrypt (int nrounds, u32 * data, int N)
{
  ...
  const u32 *rk = d_rk;         // to avoid changing d_rk in the code
  u32 aux;

  int myID = blockIdx.x * blockDim.x + threadIdx.x;
  // check if there is a block to process
  if (myID >= (N >> 4))
    return;
   ...
   */
  aux = data[myDataIdx]; s0 = RESHUFFLE(aux) ^ rk[0];
  aux = data[myDataIdx+1]; s1 = RESHUFFLE(aux) ^ rk[1];
  aux = data[myDataIdx+2]; s2 = RESHUFFLE(aux) ^ rk[2];
  aux = data[myDataIdx+3]; s3 = RESHUFFLE(aux) ^ rk[3];
